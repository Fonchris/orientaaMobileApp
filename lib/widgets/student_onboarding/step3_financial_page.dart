...
class Step3FinancialPage extends StatelessWidget {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                ...
                Expanded(
                  child: Text(
                    'We ask for financial information to recommend universities, '
                    'scholarships, and student loan options that match your budget. '
          ...
          // ── Budget Range ──
          _sectionLabel('Annual Fee / Budget Range', isDark),
          const SizedBox(height: 4),
          Text(
          ...
          Row(
            children: [
              _sectionLabel('Annual Household Income', isDark),
          ...
          _buildDropdown<String>(
            ...
            onChanged: (v) {
              if (v != null) {
                // Parse the bracket to a rough estimate for storage
                final bracket = v;
                double? parsedValue;
                if (bracket.startsWith('Less than')) {
                  parsedValue = 2500;
                } else if (bracket.startsWith('Over')) {
                  parsedValue = 100000;
                } else if (bracket == 'Prefer not to say') {
                  parsedValue = 0;
                } else {
                  // Extract the lower bound
                  final parts = bracket.split(' – ');
                  if (parts.isNotEmpty) {
                    final lower = parts[0].replaceAll('\$', '').replaceAll(',', '');
                    parsedValue = double.tryParse(lower);
                  }
                }
            ...
            displayValue: model.annualIncome != null
                ? _incomeDisplayValue(model.annualIncome!)
                : null,
          ),
          const SizedBox(height: 20),

          // ── Scholarship Seeking ──
          _sectionLabel('Are you seeking scholarships?', isDark),
          const SizedBox(height: 8),
                  ...
                  child: ElevatedButton(
                    ...
                    child: Text(
                      ...
                      style: GoogleFonts.plusJakartaSans(
                        fontSize: 16,
                        fontWeight: FontWeight.w600,
                        color: model.step3Valid ? Colors.white : subtitleColor,
                      ),
                    ),
                  ),
                ),
  ...
  String _incomeDisplayValue(double income) {
    if (income == 0) return 'Prefer not to say';
    if (income == 2500) return 'Less than \$5,000';
    if (income == 100000) return 'Over \$100,000';
    for (final bracket in incomeBrackets) {
      if (bracket.contains('\$${income.toStringAsFixed(0)}')) {
        return bracket;
      }
    }
    return 'Selected';
  ...
  // ── Budget Selector (horizontal chip list) ──
